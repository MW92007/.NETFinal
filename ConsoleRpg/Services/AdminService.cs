...
public class AdminService
{
    ...
    public ServiceResult MostDangerousRoom()
    {
        var dangerData = _context.Monsters
    ...
    .FirstOrDefault();
        ...
        if (dangerData == null)
            return ServiceResult.Ok("No monsters found in any rooms.");

        var room = _context.Containers
            .OfType<Room>()
            .FirstOrDefault(r => r.Id == dangerData.RoomId);

        if (room == null)
            return ServiceResult.Fail("Room data could not be found.");
    ...
    $"{room.Name} is the most dangerous room with {dangerData.TotalHp} total monster HP.");
    }

    public ServiceResult InventoryAudit()
    {
        var groupedItems = _context.Items
        .Include(i => i.Container)
        ...
        if (!groupedItems.Any())
            return ServiceResult.Ok("No items found in the database.");

        foreach (var group in groupedItems)
        {
            var table = new Table()
                .Border(TableBorder.Rounded)
                .Title($"[yellow]{Markup.Escape(group.Key)}[/]")
                .AddColumn("[cyan]Item Name[/]");
